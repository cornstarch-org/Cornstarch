...
class TestMultimodalModelPolicy(PolicyTestBase):
    ...
    def build_model_from_muiltimodal_plugin(
        ...
        precision = torch.float32 if precision == "fp32" else torch.bfloat16
        vision_model: PreTrainedModel = self.model_fn(
            vision_config, model_class_dict[vision_model_name]
        )
        language_model: PreTrainedModel = self.model_fn(
            language_config, model_class_dict[language_model_name]
        )

        org_model = MultimodalModel(
            encoders={"vision": ModalModule(vision_model)},
            language_model=language_model,
        ).to(dtype=precision, device=torch.device("cuda"))
        org_model.train()
        ...
        vision_plugin = ModalParallelPlugin(
            ...
            pipeline_template=get_pipeline_template(
                vision_config, vision_model_name, vision_pp_size
            ),
        )
        language_plugin = ModalParallelPlugin(
            tp_size=test_config.pop("language_tp_size"),
            pipeline_template=get_pipeline_template(
                language_config, language_model_name, language_pp_size
            ),
        )
    ...
    def run_forward_backward_with_multimodal_plugin(
        ...
        torch.Tensor, CausalLMOutputWithPast, torch.Tensor, CausalLMOutputWithPast
    ]:
        org_model.to("cuda")
