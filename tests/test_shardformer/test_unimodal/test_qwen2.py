...
@instantiate_parametrized_tests
class TestQwen2ModelPolicy(ColossalaiHybridParallelBase):
...
@instantiate_parametrized_tests
class TestQwen2ForCausalLMPolicy(ColossalaiHybridParallelBase):
    @parametrize(
        "tp_size, pp_size",
        [(4, 1), (1, 1), (2, 2), (1, 4)],
        name_fn=lambda tp, pp: f"tp{tp}_pp{pp}",
    ...
    def test_hybrid_parallel(
        ...
        self.set_model(Qwen2ForCausalLMBase())
        self.run_hybrid_parallel(tp_size, pp_size, None, fa, precision)
