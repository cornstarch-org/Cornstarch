...
def calculate_dataset_perplexity(model, dataloader_val, device: str = "cuda"):
    """
...
def finetune(
    ...
    lr_llm: Optional[float] = 5e-6,  # 1e-5,
    ...
    for epoch in range(num_epoch):
        ...
        with tqdm(
            ...
            avg_nll, perplexity = calculate_dataset_perplexity(
                model,
                dataloader_val,
                device="cuda",
            )
            ...
            f_log.write(f"[result] Epoch {epoch + 1} loss: {loss2}\n")
            f_log.write(f"[result] Epoch {epoch + 1} nll: {avg_nll}\n")
            f_log.write(f"[result] Epoch {epoch + 1} perplexity: {perplexity}\n")
