"""
An example of pretraining a vision-language model (VLM) using DistributedDataParallel (DDP).
...
def pretrain(
    ...
    optimizer = Adam(
        [p for p in ddp_model.parameters() if p.requires_grad], lr=1e-5, fused=True
    )
    optimizer.zero_grad()
