...
class OneForwardOneBackwardSchedule(PipelineSchedule):
    ...
    def load_batch(
        self, data_iter: Iterable, device: Optional[torch.device] = None
    ) -> None:
        ...
        if self.microbatch_size is None:
            ...
            ), "Batch size should divided by # microbatches"
        ...
        if not self.forward_only:
            assert (
                self.last_batch_size is None or self.last_batch_size == self.batch_size
            )
    ...
    def recv_forward(self, prev_rank: int = None) -> Any:
        ...
        if not self.stage_manager.is_first_stage(check_only_in_modal=False):
            input_tensor, _ = self.comm.recv_forward(
    ...
    def recv_backward(self, next_rank: int = None) -> Any:
        ...
        if not self.stage_manager.is_last_stage(check_only_in_modal=False):
            output_tensor_grad, _ = self.comm.recv_backward(
            ...
            if "backward_tensor_keys" not in output_obj:
                ...
                for k in output_obj["backward_tensor_keys"]:
                    ...
                    optimizer.backward_by_grad(
                        tensor_to_backward, tensor_to_backward.grad
                    )
    ...
    def run_forward_backward(
        ...
        num_warmup_microbatches = (
            self.stage_manager.num_stages - self.stage_manager.stage - 1
        )
    ...
    def forward_backward_step(
        ...
        if self.forward_only:
            result = self.run_forward_only(
                model, data_iter, criterion, return_loss, return_outputs
            )
        else:
