# Copied from https://github.com/hpcaitech/ColossalAI/blob/v0.3.5/colossalai/shardformer/policies/t5.py
...
class T5BasePolicy(Policy):
    ...
    def module_policy(self):
        ...
        if self.shard_config.enable_sequence_parallelism:
            ...
            warnings.warn(
                "T5 doesn't support sequence parallelism now, will ignore the sequence parallelism flag."
            )
    ...
    def distribute_t5_layers(
        ...
        if num_encoder_layers <= 0:
            raise ValueError(
                "The number of encoder layers for T5 must be a positive integer."
            )

        # number of layers should be large enough to fill in every stage
        if num_encoder_layers + num_decoder_layers < num_stages:
            raise ValueError(
                "The total number of layers can't be smaller than number of stages."
            )
        ...
        def objective(num_encoder_stages):
            return abs(
                num_encoder_layers / num_encoder_stages
                - num_decoder_layers / (num_stages - num_encoder_stages)
            )

        num_encoder_stages = np.argmin([objective(i) for i in range(1, num_stages)]) + 1
        num_decoder_stages = num_stages - num_encoder_stages

        encoder_distribution = Policy.distribute_layers(
            num_encoder_layers, num_encoder_stages
        )
        decoder_distribution = Policy.distribute_layers(
            num_decoder_layers, num_decoder_stages
        )
    ...
    def get_t5_stage_index(
        ...
        if stage < decoder_starting_stage:
            return Policy.get_stage_index(
                layers_per_stage[:decoder_starting_stage], stage
            )
        else:
            return Policy.get_stage_index(
    ...
    def set_pipeline_forward(
        ...
        stage_index = T5BasePolicy.get_t5_stage_index(
            layers_per_stage, stage_manager.stage, decoder_starting_stage
        )
    ...
    def module_policy(self):
        ...
        if self.pipeline_stage_manager is not None:
            self.set_pipeline_forward(
                model_cls=T5Model,
                new_forward=T5PipelineForwards.t5_model_forward,
                policy=policy,
            )
    ...
    def get_shared_params(self) -> List[Dict[int, Tensor]]:
        ...
        if stage_manager is not None and stage_manager.num_stages > 1:
            ...
            if id(module.decoder.embed_tokens.weight) == id(module.shared.weight):
                ...
                shared_embedding[decoder_starting_stage] = (
